...
## 2.5 功能体检报告(Plan 39,常驻资产)

`node test/feature-report.mjs` —— 逐 `test/feature-manifest.json` 项 e2e 捕获截图/webm + 生成
单文件 `test/results/feature-report.html`(暗色/目录/内嵌,双击即看)。**报告即测试**:任一项
捕获断言失败即非 0 退出。**每个大 plan 收口后重跑一次**(功能不腐、介绍常新);增量
`--only <group|id>`;仅重出报告 `--report-only`。清单 schema 由 unit 层测试锁死(在五层门内);
捕获与报告本体 gitignore(重跑即得),manifest/生成器/spec 入 git。
