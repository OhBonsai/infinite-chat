import { m as N } from "./boot-B4nH8qNC.js";
import { loadReplayConfig as j, REPLAY_CASES as D, saveReplayConfig as H } from "./replay-config-BGpffVO3.js";
const R = 60, V = [0.25, 0.5, 1, 2, 4];
function W(t, r) {
  t.set_reveal_cps(R), t.set_reveal_slow(1);
  let a = false, s = false, k = 1, l = 0, m = 4e3, x = 0, h = 0, p = -1;
  ...
  }, C = T("\u23EE"), u = T("\u25B6"), A = T("\u23EA"), B = T("\u23E9"), b = document.createElement("select");
  ...
  w.style.cssText = "font:11px ui-monospace,monospace;color:#9399b2", g.append(P, d, w), r.appendChild(g);
  const _ = 1e3 / 30, E = (o) => `${(o / 1e3).toFixed(2)}s`, $ = () => {
    w.textContent = `${E(l)} / ${E(m)}  ${s ? "\u25B6" : "\u23F8"} ${k}\xD7`;
  }, M = () => {
    var _a;
    const o = ((_a = t.stats()) == null ? void 0 : _a.glyphsTotal) ?? 0;
    m = Math.max(800, Math.round(o > 0 ? o / R * 1e3 + 800 : 4e3)), d.max = String(m);
  }, y = (o) => {
    ...
    if (!s) return;
    const f = Math.min(64, o - x) * k;
    if (x = o, p >= 0 && n(), l >= m) {
      i();
      return;
    }
    t.tick(f), y(l + f), h = requestAnimationFrame(F);
  }, e = () => {
    v(), !s && (s = true, u.textContent = "\u23F8", l >= m && (t.seek_reveal(0), y(0)), x = performance.now(), h = requestAnimationFrame(F));
  };
  function i() {
...
function K(t, r = document.body) {
  ...
  const k = s.getContext("2d"), l = document.createElement("div"), m = (e, i) => {
  ...
  }, x = j(), h = "flex:1;font:11px ui-monospace,monospace;color:#cdd6f4;background:#313244;border:0;border-radius:4px;padding:3px;cursor:pointer", p = (e, i, S, L) => {
    const o = document.createElement("option");
    o.value = i, o.textContent = S, o.selected = L, e.append(o);
  ...
  B.style.cssText = "display:flex;margin-top:6px";
  let b = false;
  const d = m("\u25A6 geometry", () => {
  ...
  _.append(s, l, A, w, B);
  let E = localStorage.getItem(I) !== "0";
  const $ = Y("debug");
