const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/debug-panel-BwkjXf2a.js","assets/boot-B4nH8qNC.js","assets/replay-config-BGpffVO3.js","assets/style-panel-BzpsWHVc.js","assets/scenes-DVjnmMym.js","assets/director-CgSS3eI1.js"])))=>i.map(i=>d[i]);
import { _ as d, b as I, __tla as __tla_0 } from "./boot-B4nH8qNC.js";
Promise.all([
  (() => {
    try {
      return __tla_0;
  ...
  async function A() {
    const n = new URLSearchParams(location.search), i = n.get("server") ?? void 0, c = n.get("session") ?? void 0, l = !i, { loadReplayConfig: h } = await d(async () => {
      const { loadReplayConfig: e } = await import("./replay-config-BGpffVO3.js");
      return {
        loadReplayConfig: e
      };
    }, []), _ = h(), v = n.get("replay"), m = v ?? _.case ?? (l ? "showcase" : void 0), g = Number(n.get("speed") ?? "") || _.speed || 1;
    let p;
    if (m) try {
      p = await (await d(async () => {
      ...
      console.warn(`[replay] \u52A0\u8F7D\u5931\u8D25,\u8DF3\u8FC7\u91CD\u653E: ${m}`, e);
    }
    n.has("empty") && (p = [
      {
        t: 0,
        raw: '{"type":"design-review.noop"}'
      }
    ]);
    const f = n.has("bench");
    let u = 0;
    ...
    const { chat: o, wasmModule: w } = await I({
      replay: p,
      serverUrl: i,
      sessionId: c,
      rhythmPreset: f ? void 0 : "reader"
    });
    if (i) {
    ...
    if (!n.has("noinput") && !l) {
      const { mountChatInput: e, parseModel: s } = await d(async () => {
        const { mountChatInput: a, parseModel: t } = await import("./chat-input-DZ_x24Jy.js");
        return {
          mountChatInput: a,
          parseModel: t
        };
    ...
    if (n.has("debug")) {
      ...
      const { mountDebugPanel: s } = await d(async () => {
        const { mountDebugPanel: a } = await import("./debug-panel-BwkjXf2a.js");
        return {
          mountDebugPanel: a
        };
      ...
      const { mountStylePanel: r } = await d(async () => {
        const { mountStylePanel: a } = await import("./style-panel-BzpsWHVc.js");
        return {
          mountStylePanel: a
        };
      }, __vite__mapDeps([3,1]));
    ...
    if (n.has("msdf") || n.has("asciimsdf")) {
      const { loadMsdf: e } = await d(async () => {
        const { loadMsdf: a } = await import("./boot-B4nH8qNC.js").then(async (m2) => {
          await m2.__tla;
          return m2;
        }).then((t) => t.o);
        return {
    ...
    if (f) {
      o.set_stream_rate(1e9), o.set_reveal_cps(Number.POSITIVE_INFINITY), n.has("sizefold") && o.set_bench_fold_width(true), n.has("novirt") && o.set_virtualize(false), window.__wasmMemory = w.memory;
      const e = [];
      let s = -1, r = 0;
      const a = setInterval(() => {
        var _a;
        const t = o.stats(), x = w.memory.buffer.byteLength, b = {
...
`);
            window.__benchCSV = y, console.log(`[bench] done \u2014 CSV (window.__benchCSV):
${y}`), (_a = navigator.clipboard) == null ? void 0 : _a.writeText(y).catch(() => {
            }), clearInterval(a);
          }
    ...
    if (console.info("[harness] ChatCanvas started", {
      ...
      const { mountFilm: e } = await d(async () => {
        const { mountFilm: s } = await import("./scenes-DVjnmMym.js");
        return {
          mountFilm: s
        };
      }, __vite__mapDeps([4,5]));
  ...
  function N() {
    ...
    const c = (l, h) => `<a href="${h}" style="pointer-events:auto;color:#3df5d0;text-decoration:none;border:1px solid #3df5d066;border-radius:6px;padding:3px 9px">${l}</a>`;
