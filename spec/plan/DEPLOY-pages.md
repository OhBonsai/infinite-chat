...
- **四页站**(Plan 41:首页回归**播放器模式**;chat/markdown/gallery 独立页作深链目标):
  - **landing`/`** —— 全屏单 canvas **幕式播放器**(引擎放映自己的宣传片):7 幕(标题 / 是什么 /
    功能巡礼 / 完整对话 / Markdown / 效果 / 入口)自动播循环 + 可接管(←/→ 键 · 进度点 · 滚轮 · 触摸横滑 ·
    暂停;接管后停自动、空闲 30s 恢复)。薄壳 DOM = 右上极简 nav + 字幕层 + 进度点 chrome + S7 入口浮层。
    无 WebGPU/WebGL2 → **pages-assets 截图/短片幻灯降级**(同幕结构/chrome);reduced-motion 不自动播。
...
- **精选资产(`web/public/pages-assets/`,入库)**:Plan 40 生成的截图/短片。**CI 无 GPU,不能现场捕获
  → 必须本地生成并提交**。与 plan39 功能体检同源同 DSL:manifest 标 `pages: true` 的精选项,单命令
  `node scripts/gen-pages-assets.mjs`(需本地 GPU + Playwright)捕获 → 拷进 `pages-assets/` + 写
  `index.json` + 体积对账(≤15MB)。**Plan 41 职责转变**:landing 已改播放器(不再渲功能卡),这些资产
  现作首页**无 GPU 幻灯降级**的数据源(每幕一张,`home-fallback.ts` 映射)。改引擎观感后重跑并提交。
