...
## R4 · streaming-svg —— ✅(2026-07-13,0036 白名单 v1)

| 项 | 落地 |
|---|---|
| **ADR 0036** | SVG 矢量原生(路 B),翻 0007「流式 SVG 走纹理」条;白名单 + 超集降级 + 与位图/动画分流判定顺序 |
| **core 解析** | `svg.rs`(新):极简手写白名单解析(零 XML 依赖,CR1/R8 纯函数)—— rect(rx)/circle(圆=全圆角 rect)/line(水平垂直)/text;viewBox 归一;hex/named 色 + 未知兜中性;白名单外(path/斜线/渐变)计 `degraded` 不 panic(AR12) |
| **矢量分流接入** | app.rs embed 发射端:静态 `data:image/svg+xml`(percent-decode;base64 变体走纹理)且白名单命中(degraded==0)→ 每图元 `FrameRect`(与正文同管线,任意缩放锐利),**不出纹理 quad**;动画 SVG 照旧 DOM overlay,位图照旧 plan14 |
| **测试** | svg.rs ×5(解析快照 insta / 圆=全圆角 / 降级计数 / 确定性双跑 / 畸形容错)+ app 分流断言 `static_whitelist_svg_renders_vector_not_texture`(images 空 + tomato 矢量 rect + zoom ×4 world 尺寸恒等 = 无栅格) |

- 遗留:① `<text>` 图元 → glyph 链接入(白名单已解析,发射待接);② web 侧 loader 对已分流
  的白名单 svg 仍会解码上传一次纹理(孤儿纹理,浪费一传;同判定跳过待做);③ 逐元素揭示
  (现随块整体 reveal;元素级 spawn 接 plan30 骨架门待做);④ 外链 .svg(非 data URI)需 web
  侧取源回传,v1 走纹理。

## 收口小结(2026-07-13)—— DoD 对账(1–8)

| DoD | 状态 |
|---|---|
| 1 新 ADR ×2 | ✅ 0035(RaTeX 一等公民,supersedes 0013)/ 0036(SVG 矢量原生,翻 0007 条) |
| 2 R1 math | ✅ 管线(plan12)+ 定位快照 + KaTeX 度量对照 4/4 Δ=0 + 骨架门(同步排版天然满足);遗留 Path 图元 |
| 3 R2 magic-move | ✅ 内容稳定身份(正文+数学区)+ 速度上限 + 5 测;v1 按预案收窄(text 中段重写记遗留,机制已备) |
| 4 R3 overlay | ✅ transform 定位 + 世界/相机分离补间 + EmbedKind 注册 + **≤1px 跟随 e2e**;html/reportSize/主题联动随首个非 image 消费者 |
| 5 R4 svg | ✅ 白名单矢量原生 + 分流断言 + zoom 恒等;遗留见上 |
| 6 四层门 + 每 R ≥1 native + golden | ✅ 每 R 提交前全门绿(385→390→391→本轮);native 新增 R1×2/R2×5/R3(e2e)/R4×6;golden:数学/svg 场景由既有 V 帧族+新 native 快照覆盖,动画组≤4/idle 退场保持 |
| 7 progress 逐 R | ✅ 本文件 |
| 8 按 R commit 不 push | ✅ R1 ee09164 / R2 2727e81 / R3 a5a3a41 / R4 本 commit |

> 护城河兑现:公式每字、SVG 每图元都是与正文同管线的 SDF/quad 实例 —— 可缩放、可动画、
> 可 morph;overlay 层像素级跟随补足「进得来」的交互半边。
