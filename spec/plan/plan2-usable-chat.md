...
| 验证点 | 预期 | 实测 |
|---|---|---|
| 长对话(10k 行)fps/内存曲线 | 平坦(vs Plan1 劣化) | 架构已落地:块冻结 + 视口裁剪 → 每帧重排/出 glyph 只与"可见+尾部"成正比;浏览器 fps/内存曲线待真机 benchmark |
| 块冻结后尾部重排开销 | 只与尾部增长成正比 | ✓ counting-layout 测试:settled 块不再 layout |
| pretext 真排版 vs measureText 度量差 | 无可见回归 | measureText 桥仍在用(body 度量);真 pretext per-role 度量推迟 Plan2.5(见 §6 偏差) |
| 流式 markdown 闪烁 | remend 后消除 | ✓ remend 补全未闭合 `**`/`` ` ``/```` ``` ````(content 测试) |
| 忘了 idle 的收尾延迟 | ≤30s 自动解禁 | ✓ TurnTracker 看门狗(fsm 6 测 + app 集成) |
| 重连快照修复 | 不闪不错,状态收敛快照 | ✓ resync 只补缺不动 live;AR4 收敛/幂等测试;周期 resync 已接 |
...
## 5.5 实施状态(Plan 2 落地记录)

- 日期:2026-06-14。相位 F–J 全部实现、过卡口、各自一个提交(英文 commit)。
- 卡口全绿:`cargo fmt --check`、`cargo clippy --workspace & --target wasm32 -- -D warnings`、
  `cargo test`(**45 native 测试**,含 proptest + 确定性重放)、`cargo build --target wasm32`。

| 相位 | 落地 | 验证 |
|---|---|---|
| **F** 快照/过滤 | protocol `parse_snapshot`;store `apply_snapshot` + partID→messageID→sessionID 归属;app `prime_from_snapshot`(零淡入 AR6)+ session 过滤;wasm 连 SSE 前先拉快照 | 快照灌入/归属/过滤/instant 单测 |
| **G** 滚动/裁剪/冻结 | per-block 排版缓存(冻结)、视口裁剪、scroll_offset + 锚底;wasm 滚轮监听 | block-freeze(counting-layout)+ cull 测 |
| **H** markdown | `pulldown-cmark` → 角色 span(隐藏语法)、remend 防闪;FrameGlyph.style → 着色器上色、atlas 按 (style,cluster) 分桶;web 按角色选字体 | 7 content 测 + naga shader |
| **I** 收尾 | `TurnTracker` 投影 + soft/hard 看门狗;session.status 解码;`Engine::turn_status()` | 收尾矩阵 6 测 + 集成 |
| **J** 容错 | `resync_from_snapshot`(只补缺不动 live);wasm 周期 resync + EventSource 自动重连;AR4 收敛/幂等测试 | 故障收敛 + 幂等 + resync 测 |

**有据偏差 / 推迟项**(均在代码注释标注):
- **真 pretext per-role 度量**:H4 的精确粗/斜/code 排版宽度推迟 Plan2.5(measureText 桥按 body 字体度量;光栅化已按角色选字体,视觉粗体正确,宽度略偏)。
- **syntect 语法高亮**(H5):重依赖,推迟(标注独立可插)。
- **Turn 完整分组投影(AR11)+ 折叠 tool/reasoning(I5)**:本期做收尾判定(最痛项),分组/折叠推迟。
- **显式心跳看门狗强制重连(J2)**:用"周期 resync + EventSource 自动重连"覆盖等价效果,显式 backoff 重连推迟。
- **可视滚动条 + 块内 glyph 级裁剪(G)**:块级裁剪已足够平坦;细化推迟。
- **delta 孤儿缓冲(J3)**:本模型 delta 乐观建 part,delta-before-updated 天然可用,无需独立缓冲。

**待真机验证**:像素/markdown 观感、≥60fps、10k 行 fps/内存曲线、滚动手感、重连观感。
