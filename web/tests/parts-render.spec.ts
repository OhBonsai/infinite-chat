...
test("TC-F05 tool 卡状态变迁(pending→done,徽章随 status,旧态不残留)", async ({ page }) => {
  // Plan 28 R3 tool 卡:显示名 trigger 行(Shell + 副题);文本徽章退役,pending/running 藏正文。
  ...
  await expect
    ...
    .toContain("Shell"); // pending:只有 trigger 行
  ...
  expect(runs, "工具显示名可见").toContain("Shell");
  expect(runs, "完成态出 $ cmd 正文").toContain("$ ");
