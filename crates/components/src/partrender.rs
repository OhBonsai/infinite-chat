...
use infinite_chat_primitives::style::{plain, StyleRole, StyledSpan};
...
pub struct RenderCtx {
    ...
    pub unfolded: u64,
    /// markdown 解析注入(0039:解析主体在 core,组件经此使用;fn 指针保 Copy/R8)。
    /// Default = `plain`(纯文本直通)——生产构造点(app)必须显式注入真 parse。
    pub parse: fn(&str) -> Vec<StyledSpan>,
}
...
impl Default for RenderCtx {
    fn default() -> Self {
        Self {
            ...
            folded: false,
            unfolded: 0,
            parse: infinite_chat_primitives::style::plain,
        }
    }
}
...
// 0039 分层:装饰契约类型(0037)已下沉 primitives(纯数据);镜像保内外路径不变。
pub use infinite_chat_primitives::decoration::{DecorKind, DecorSlot, DecorationOp, RenderOutput};

/// full 渲染器签名(0037):同为纯函数(CR1/R8)。
pub type FullRenderFn = fn(PartKind, &RenderPart, &RenderCtx) -> RenderOutput;

/// 通用兜底渲染器(Plan 22):任何 part → **身份标签 + 原始内容**(markdown / JSON 代码块)。
/// 丑但完整 —— 肉眼能看出「是什么 + 内容是什么」。Plan 23 用 specific 渲染器逐类覆盖。
#[must_use]
pub fn fallback_render(kind: PartKind, part: &RenderPart, ctx: &RenderCtx) -> Vec<StyledSpan> {
    ...
    match kind {
        ...
        PartKind::Text | PartKind::Reasoning => out.extend((ctx.parse)(&part.text)),
...
#[cfg(test)]
pub fn assert_renderfn_conforms(f: RenderFn) {
...
#[cfg(test)]
pub fn adversarial_render_parts() -> Vec<(PartKind, RenderPart)> {
