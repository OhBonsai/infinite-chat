...
// tool-icon 专用助手(plan16-tool-icons §2):点到线段距离 / 居中偏移 / 线段揭示。deck 老盘不用,
// 故只在 icons.wgsl 内定义(不污染 common.wgsl)。`sb_seg` 避与 polySDF 内局部 `seg` 同名。
fn sb_seg(p: vec2<f32>, a: vec2<f32>, b: vec2<f32>) -> f32 {
    let pa = p - a;
    let ba = b - a;
...
fn line2(p: vec2<f32>, a: vec2<f32>, b: vec2<f32>, w: f32) -> f32 { return 1.0 - sstep(w, sb_seg(p, a, b)); }
...
fn icon_draw(uv: vec2<f32>, icon: i32, t: f32) -> f32 {
    ...
    switch icon {
        ...
        // ── opencode 内置 tool 的 animated icon(plan16-tool-icons,id 50..65;全 dynamic)──
        case 50: { // read — 纸张 + 扫描线下移
            let page = rectSDF(st, vec2<f32>(0.58, 0.78));
            let inside = sb_fill(page, 0.5);
            color = sb_stroke(page, 0.5, 0.045);
            let scan_y = 0.80 - 0.60 * fract(t * 0.6);
            color += sb_stroke(st.y, scan_y, 0.022) * inside;
        }
        case 51: { // write — 纸张 + 笔尖横移留迹 + caret 闪
            let page = rectSDF(st, vec2<f32>(0.58, 0.78));
            let inside = sb_fill(page, 0.5);
            color = sb_stroke(page, 0.5, 0.045);
            let pen_x = 0.30 + 0.40 * fract(t * 0.5);
            let base_y = 0.40;
            color += sb_stroke(st.y, base_y, 0.020) * step(st.x, pen_x) * step(0.28, st.x) * inside;
            let caret = sb_fill(rectSDF(offc(st, pen_x, base_y + 0.06), vec2<f32>(0.020, 0.14)), 0.5);
            color += caret * step(0.5, fract(t * 2.0)) * inside;
        }
        case 52: { // edit — 文本行 + 高亮框滑动反相
            var lines = sb_stroke(st.y, 0.64, 0.020) * step(0.24, st.x) * step(st.x, 0.76);
            lines += sb_stroke(st.y, 0.50, 0.020) * step(0.24, st.x) * step(st.x, 0.68);
            lines += sb_stroke(st.y, 0.36, 0.020) * step(0.24, st.x) * step(st.x, 0.72);
            let hcx = 0.34 + 0.30 * fract(t * 0.35);
            let hbox = rectSDF(offc(st, hcx, 0.50), vec2<f32>(0.17, 0.10));
            color = sb_flip(lines, sb_fill(hbox, 0.5));
            color += sb_stroke(hbox, 0.5, 0.03);
        }
        case 53: { // grep — 放大镜 + 镜内文本上滚
            let lens = circleSDF(st);
            let ins = sb_fill(lens, 0.44);
            color = sb_stroke(lens, 0.46, 0.05);
            let h = sb_rotate(st, radians(-45.0));
            color += sb_stroke(h.x, 0.5, 0.035) * step(0.50, h.y) * step(h.y, 0.80);
            for (var i: i32 = 0; i < 3; i = i + 1) {
                let line_y = 0.30 + 0.40 * fract(0.20 + f32(i) * 0.30 + t * 0.25);
                color += sb_stroke(st.y, line_y, 0.022) * ins * step(0.32, st.x) * step(st.x, 0.68);
            }
        }
        case 54: { // glob — 星号 * 旋转 + 角落命中方块闪
            let rays = raysSDF(sb_rotate(st, t * 0.3), 6);
            let disc = circleSDF(st);
            color = sb_stroke(rays, 0.5, 0.085) * sb_fill(disc, 0.66);
            color += sb_fill(disc, 0.12);
            for (var i: i32 = 0; i < 4; i = i + 1) {
                let a = f32(i) * SB_TAU / 4.0 + SB_PI / 4.0;
                let p = vec2<f32>(cos(a), sin(a)) * 0.40;
                let blink = step(0.5, fract(t * 0.6 + f32(i) * 0.25));
                color += sb_fill(rectSDF(st - p, vec2<f32>(0.06, 0.06)), 0.5) * blink * 0.9;
            }
        }
        case 55: { // shell — ">" + 下划线 caret 闪
            let f_u = (st.x + st.y) * 0.5;
            let f_l = 0.5 + (st.y - st.x) * 0.5;
            let bx = step(0.28, st.x) * step(st.x, 0.66);
            let by_u = step(0.50, st.y) * step(st.y, 0.74);
            let by_l = step(0.26, st.y) * step(st.y, 0.50);
            color = sb_stroke(f_u, 0.58, 0.055) * bx * by_u;
            color += sb_stroke(f_l, 0.42, 0.055) * bx * by_l;
            color += sb_stroke(st.y, 0.30, 0.030) * step(0.70, st.x) * step(st.x, 0.88) * step(0.5, fract(t * 1.5));
        }
        case 56: { // task — 枢纽 + 轨道 + 3 卫星环绕
            color = sb_stroke(circleSDF(st), 0.66, 0.012);
            color += sb_fill(circleSDF(st), 0.17);
            for (var i: i32 = 0; i < 3; i = i + 1) {
                let a = t * 0.8 + f32(i) * SB_TAU / 3.0;
                let p = vec2<f32>(cos(a), sin(a)) * 0.33;
                color += sb_fill(circleSDF(st - p), 0.11);
            }
        }
        case 57: { // webfetch — 地球仪 + 点向心飞入
            let globe = circleSDF(st);
            let inside = sb_fill(globe, 0.62);
            color = sb_stroke(globe, 0.62, 0.04);
            color += sb_stroke(circleSDF(sb_scale(st, vec2<f32>(2.4, 1.0))), 0.62, 0.03) * inside;
            color += sb_stroke(st.y, 0.5, 0.03) * inside;
            let dx = 0.5 + 0.45 * (1.0 - fract(t * 0.6));
            color += sb_fill(circleSDF(offc(st, dx, 0.5)), 0.07) * step(0.52, dx);
        }
        case 58: { // websearch — 声呐扩散环
            let r = circleSDF(st);
            for (var i: i32 = 0; i < 3; i = i + 1) {
                let phase = fract(t * 0.5 + f32(i) / 3.0);
                color += sb_stroke(r, phase * 1.15, 0.035 * (1.0 - phase));
            }
            color += sb_fill(r, 0.11);
        }
        case 59: { // todowrite — 复选框 + 顶行对勾描出
            for (var i: i32 = 0; i < 3; i = i + 1) {
                let fy = 0.66 - f32(i) * 0.18;
                let cbox = rectSDF(offc(st, 0.30, fy), vec2<f32>(0.11, 0.11));
                color += sb_stroke(cbox, 0.5, 0.035);
                color += sb_stroke(st.y, fy, 0.020) * step(0.46, st.x) * step(st.x, 0.80);
            }
        ...
        case 60: { // skill — 钥匙微转
            let k = sb_rotate(st, sin(t * 0.8) * 0.12);
            color = sb_stroke(circleSDF(offc(k, 0.30, 0.5)), 0.30, 0.05);
            color += sb_stroke(k.y, 0.5, 0.045) * step(0.38, k.x) * step(k.x, 0.80);
            color += sb_stroke(k.x, 0.66, 0.030) * step(0.40, k.y) * step(k.y, 0.50);
            color += sb_stroke(k.x, 0.74, 0.030) * step(0.40, k.y) * step(k.y, 0.50);
        }
        case 61: { // apply_patch — diff ± 交替加重
            color = sb_stroke(rectSDF(st, vec2<f32>(0.66, 0.66)), 0.5, 0.030);
            let e_u = step(0.5, fract(t * 0.6));
            color += sb_fill(crossSDF(offc(st, 0.5, 0.63), 1.0), 0.15) * (0.4 + 0.6 * e_u);
            color += sb_stroke(st.y, 0.37, 0.045) * step(0.34, st.x) * step(st.x, 0.66) * (0.4 + 0.6 * (1.0 - e_u));
        }
        case 62: { // question — "?" 呼吸
            let q = offc(st, 0.5, 0.60);
            let ang = atan2(q.y - 0.5, q.x - 0.5);
            color = sb_stroke(circleSDF(q), 0.30 * sb_S(t), 0.050) * step(-2.3, ang);
            color += sb_stroke(st.x, 0.5, 0.050) * step(0.34, st.y) * step(st.y, 0.48);
            color += sb_fill(circleSDF(offc(st, 0.5, 0.26)), 0.055);
        }
        case 63: { // plan-enter — 折线路由 draw-on
            ...
            let tt = fract(t * 0.4);
            color += line2(st, n0, mix(n0, n1, clamp(tt * 2.0, 0.0, 1.0)), 0.020);
            color += line2(st, n1, mix(n1, n2, clamp(tt * 2.0 - 1.0, 0.0, 1.0)), 0.020) * step(0.5, tt);
            color += sb_fill(circleSDF(offc(st, n0.x, n0.y)), 0.07);
            color += sb_fill(circleSDF(offc(st, n1.x, n1.y)), 0.07) * step(0.5, tt);
            color += sb_fill(circleSDF(offc(st, n2.x, n2.y)), 0.07) * step(0.96, tt);
        }
        case 64: { // plan-exit — 路由已成 + 播放三角呼吸
            ...
            color += sb_fill(circleSDF(offc(st, n0.x, n0.y)), 0.06);
            color += sb_fill(circleSDF(offc(st, n2.x, n2.y)), 0.06);
            color += sb_fill(triSDF(sb_rotate(offc(st, n1.x, n1.y), radians(-90.0))), 0.22 * sb_S(t));
        }
        case 65: { // invalid — 禁止符抖动
            let ring = circleSDF(st);
            color = sb_stroke(ring, 0.60, 0.05);
            let dd = 0.5 + (st.x - st.y) * 0.5;
            color += sb_stroke(dd, 0.5, 0.05) * sb_fill(ring, 0.60);
            color *= 0.55 + 0.45 * step(0.5, fract(t * 6.0 + sin(t * 13.0)));
        }
