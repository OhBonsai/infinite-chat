// image.wgsl — 图片纹理 quad(Plan 14 ②)。一个 world-space quad,fragment 采 per-image RGBA 纹理
// × alpha(0025 淡入),可选圆角裁切(复用 base/sdf.wgsl 的 sd_round_box)。warp `renderer/image.rs`
// 同范式:非 atlas,每图一组 texture+sampler bind group(group 1)。图作底、文字压上(glyph 之前)。

struct Globals {
    viewport: vec2<f32>,
    time_ms: f32,
    fade_ms: f32,
    cam_pan: vec2<f32>,
    cam_zoom: f32,
    _pad: f32,
};

@group(0) @binding(0) var<uniform> globals: Globals;
// per-image 纹理 + sampler(group 1;backend 据 tex_id 选对应 bind group)。
@group(1) @binding(0) var tex: texture_2d<f32>;
@group(1) @binding(1) var samp: sampler;

struct InstanceIn {
    @location(0) pos: vec2<f32>,   // 左上角世界 px
    @location(1) size: vec2<f32>,  // 宽高 px
    @location(2) alpha: f32,       // 不透明度 0..1(0025 淡入)
    @location(3) radius: f32,      // 圆角半径 px
};

struct VsOut {
    @builtin(position) clip: vec4<f32>,
    @location(0) uv: vec2<f32>,                       // 纹理坐标 0..1
    @location(1) local: vec2<f32>,                    // 框中心为原点的世界 px
    @location(2) @interpolate(flat) halfsz: vec2<f32>,
    @location(3) @interpolate(flat) radius: f32,
    @location(4) @interpolate(flat) alpha: f32,
};

@vertex
fn vs_main(@builtin(vertex_index) vid: u32, inst: InstanceIn) -> VsOut {
    var corners = array<vec2<f32>, 4>(
        vec2<f32>(0.0, 0.0), vec2<f32>(1.0, 0.0),
        vec2<f32>(0.0, 1.0), vec2<f32>(1.0, 1.0),
    );
    let c = corners[vid];
    let world = inst.pos + c * inst.size;
    let screen = (world - globals.cam_pan) * globals.cam_zoom;
    let ndc = vec2<f32>(
        screen.x / globals.viewport.x * 2.0 - 1.0,
        1.0 - screen.y / globals.viewport.y * 2.0,
    );
    var out: VsOut;
    out.clip = vec4<f32>(ndc, 0.0, 1.0);
    out.uv = c;
    out.local = (c - vec2<f32>(0.5, 0.5)) * inst.size;
    out.halfsz = inst.size * 0.5;
    out.radius = min(inst.radius, min(out.halfsz.x, out.halfsz.y));
    out.alpha = inst.alpha;
    return out;
}

@fragment
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    // 圆角裁切(SDF + fwidth AA);radius=0 → 直角矩形全覆盖。
    let d = sd_round_box(in.local, in.halfsz, in.radius);
    let aa = max(fwidth(d), 0.0001);
    let inside = 1.0 - smoothstep(-aa, aa, d);
    if inside <= 0.0 {
        discard;
    }
    let texel = textureSample(tex, samp, in.uv);
    return vec4<f32>(texel.rgb, texel.a * in.alpha * inside);
}
