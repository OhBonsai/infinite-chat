...
// 0039 分层:StyleRole/StyledSpan 本体已下沉 primitives(纯数据);此处镜像保 crate 内外路径不变。
pub use infinite_chat_primitives::style::{StyleRole, StyledSpan};

/// 高亮语义类 → 渲染角色(0039:StyleRole 已下沉、CodeClass 属 components 域 → 此映射留 core)(`Plain` 复用 `CodeBlock`,余 7 类各自角色)。
pub(crate) fn role_from_code_class(cls: crate::highlight::CodeClass) -> StyleRole {
